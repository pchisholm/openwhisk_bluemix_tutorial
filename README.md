...
If the word serverless is new or confusing to you just know it is a misnomer, statelessness is more contextual in the case of OpenWhisk. Serverless in this case is meant to denote the existence of an API host that activates 'dockerized' compute events per request. These events remain inactive outside of near real-time usage so rely on a persistence layer for the state machine when required. OpenWhisk uses a swagger esque syntax for managing routes/CORS that you can read about [here](https://github.com/openwhisk/openwhisk/blob/master/docs/reference.md#rest-api). This layer of abstraction makes it easy for the developer(s) to assemble a centralized web API relying on *n* number of supporting environments with very little technical overhead outside their language(s) of choice. 

If you are wondering why there is a NodeJS/Express server and CF app deployment in this repo, this is only intended to serve static content. It is possible to serve static content in a serverless fashion but to I wanted to keep this tutorial centered around how to cobble together back end assets. Some considerations being:
...
In this case, I have a made up list of tasks, found in: `root/packages/config/tasks.json` and my core requirement is to display this original task list and a random subset of 'daily tasks' via a web app. I am asked to provide an endpoint that responds with the task list data and the specifications require that the logic be written in NodeJS. I am also tasked with providing secondary endpoints that can sort the subset of tasks arbitrarily if needed and the specifications require that I use python to accomplish these tasks.
...
Each package directory and action sub-directory contains an `initialize.sh` script which will simply run the appropriate wsk deployment command. If the action is to be web exported (meaning it requires an API endpoint) the action name serves as a key and endpoint url as value that will be written to a `root/public/endpoints.json` file. In this tutorial, `root/packages/config/tasks.json` is a flat file which is meant to emulate a store of some kind. With this in mind the general flow for this deployment is: 
...
Congrats, even though you haven't added any code to index.py yet, you have just created the infrastructure for your first OpenWhisk action :cool:. The next step is to program our main python function to sort the incoming task-subset array. `index.py` should be as follows: 
...
```
{
    "subset": [
        ...
        "Add a Bluemix service to the project.",
...
If you are error free, return to the root directory and run `./setup.sh 'local | cloud'` to update the endpoints file and UI assets. If you are used to web development it should be clear from looking at `root/public/index.js` how to use the endpoints file with AJAX style requests. I suggest implementing some functionality that uses the getTasksAZ sequence if the pieces don't click there. Since this is not a front-end focused tutorial I don't plan to expand any further here. Feel free to replace whatever you want in public or completely get rid of the directory / UI deployment code in `./setup.sh` if you are just looking to create web hooks. 
...
Feel free to open issues / pull requests with suggestions & improvements, thanks!
...
- [openwhisk-objectstore: One way to use objectstore to host static and deploy a stateless front-end](https://github.com/starpit/openwhisk-objectstore)
...
- [Setting up python virtual environments with OpenWhisk](http://jamesthom.as/blog/2017/04/27/python-packages-in-openwhisk/)
